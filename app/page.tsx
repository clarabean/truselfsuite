...
// Sanitize appId to avoid slash-related Firestore path errors
...
export default function App() {
  ...
  const analyzeWithGemini = async () => {
    ...
    try {
      // Call your SECURE backend tunnel
      ...
      if (!res.ok) {
        throw new Error(aiData.error || "The Master Coach is busy. Please try again shortly.");
      }
      ...
      console.error(e);
      setErrorMessage(e.message || "Connection interrupted. Please try again.");
    } finally { 
            ...
            <div className="flex-1 w-full bg-white/5 backdrop-blur-md border border-white/10 rounded-[40px] p-8 shadow-2xl">
              ...
              {!historyOpen ? (
                <div className="space-y-6">
                  ...
                  {errorMessage && (
                    <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-2xl text-red-200 text-sm font-medium text-center">
                      {errorMessage}
                    </div>
                  )}
