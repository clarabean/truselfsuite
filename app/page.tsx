...
?key=${apiKey}`, {
      ...
      const newEntry = { ...aiData, id: Date.now().toString(), timestamp: Date.now(), date: new Date().toLocaleDateString(), text: diaryEntry };
      setDiaryHistory(prev => [newEntry, ...prev]);
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-white flex flex-col" style={{ fontFamily: "'Jost', sans-serif" }}>
                  ...
                  {diaryHistory.map(item => (
                    <div key={item.id} className="bg-white/5 border border-white/10 rounded-3xl p-6">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <span className="text-[10px] text-indigo-300 uppercase font-black">{item.topDomain}</span>
                      </div>
                      <p className="text-indigo-100/80 italic line-clamp-3 font-medium">"{item.text}"</p>
                    </div>
                  ))}
                ...
                <div className="bg-white rounded-[40px] shadow-2xl text-[#1E1B4B] overflow-hidden">
                  ...
                  <div className="p-8 space-y-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div><p className="text-[10px] font-black uppercase text-indigo-400 mb-1">Pattern Sieve</p><h3 className="text-xl font-black uppercase tracking-tighter">{diaryAnalysis.topDomain}</h3></div>
                    </div>
                    <div><h4 className="text-[10px] font-black uppercase text-indigo-300 mb-2 tracking-widest">Synthesis</h4><p className="font-bold leading-snug">{diaryAnalysis.summary}</p></div>
          ...
          <div className="relative bg-white w-full max-w-md rounded-[50px] overflow-hidden shadow-2xl">
            ...
            <div className="p-10 text-[#1E1B4B]">
              <div className="flex items-center gap-5 mb-8">
                ...
                <div><h2 className="text-2xl font-black uppercase tracking-tighter">{gachaResult.title}</h2><p className="text-[10px] text-amber-600 font-bold uppercase tracking-widest mt-2">Daily Spark</p></div>
              </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `@keyframes bounce { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-15px); } } .animate-bounce { animation: bounce 0.5s infinite; }` }} />
    </div>
  );
}